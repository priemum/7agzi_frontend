...
const SingleAppointmentPageBoss = (props) => {
	...
	const loadSingleOrder = (AppointmentId, employeeId) => {
		setLoading(true);
		readSingleAppointment(ownerId, token, AppointmentId).then((data) => {
			if (data.error) {
	...
	const loadStatusValues = () => {
		getStatusValues(ownerId, token).then((data) => {
			if (data.error) {
				console.log(data.error);
			} else {
	...
	const handleStatusChange = (e, orderId) => {
		var updatedByUser =
			isAuthenticated() && isAuthenticated().user && isAuthenticated().ownerId;
	...
	const loadOrders = () => {
		listScheduledOrders(ownerId, token, ownerId).then((data) => {
			if (data.error) {
				console.log(data.error);
			} else {
				setHistOrders(
				...
				<React.Fragment>
					<div className='row'>
						<div className='col-md-10 mt-3 mx-auto'>
							<Link
								to={`/boss/store/admin/dashboard/${ownerId}`}
								onClick={() => {
									window.scrollTo({top: 0, behavior: "smooth"});
								}}
							>
...
export default SingleAppointmentPageBoss;
