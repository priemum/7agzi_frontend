...
const SchedulePageStepsStore = () => {
	...
	const loadOrders = () => {
		...
		listScheduledOrders(userId, token, userBelongsToModified).then((data) => {
			if (data.error) {
				console.log(data.error);
			} else {
				setHistOrders(
	...
	//Getting Interval between to booked times to know the unavailibility of the stylist
	...
	const clickSubmitSchedule_NoPayment = () => {
		...
		if (storeClosed_NotClosed) {
			return toast.error("Salon is closed on the chosen date");
		}
	...
	return (
		<SchedulePage className='col-md-10 mx-auto mt-4'>
			<Helmet>
				<meta charSet='utf-8' />
				<title>Store Booking</title>
				<meta name='description' content='Store Booking' />
			</Helmet>
			<div
			...
			>
				"{alreadySetLoyaltyPointsManagement.addStoreName}" In Store Booking
			</div>
...
export default SchedulePageStepsStore;
