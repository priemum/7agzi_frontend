...
const SingleAppointmentPageStore = (props) => {
	...
	const loadStatusValues = () => {
		getStatusValuesStore(user._id, token).then((data) => {
			if (data.error) {
				console.log(data.error);
			} else {
	...
	const handleStatusChange = (e, orderId) => {
		updateOrderStatusStore(user._id, token, orderId, e.target.value).then(
			(data) => {
				if (data.error) {
		...
		getServices(token, userBelongsToModified).then((data) => {
			if (data.error) {
				...
				var pickedEmployeeServicesIncluded =
					chosenStylist &&
					chosenStylist.services &&
					chosenStylist.services.map((ii) => ii.serviceName + ii.customerType);
				setAllCustomerTypes([
					...new Set(
						data &&
							data.filter(
		...
		listScheduledOrdersStore(user._id, token, userBelongsToModified).then(
			(data) => {
				if (data.error) {
					...
					getAllEmployees(userBelongsToModified).then((data) => {
						setLoading(true);
						if (data.error) {
							console.log(data.error);
						} else {
	...
	// console.log(chosenServicePrice, "allServicesModified");
	...
	const relatedCustomersHistory = () => {
		var customersOrders =
			...
			orders[0] &&
			orders.filter((i) => i.phone === singleAppointment.phone);
		return (
							...
							{customersOrders &&
								customersOrders.map((s, i) => (
									...
									>
										...
										<td>EGP {s.serviceDetails.servicePriceDiscount}</td>
										<td>EGP {s.amount.toFixed(2)}</td>
			...
			{loading ? (
				...
				<React.Fragment>
					<div className='row mx-auto text-center my-5 col-md-10'>
						...
						<div
							className='col-md-4 mx-auto'
							style={isActive("", "OverAllCalendar")}
						>
							<Link
								style={isActive("", "OverAllCalendar")}
						...
						<div className='col-md-4 mx-auto' style={isActive("", "TableView")}>
							<Link
								style={isActive("", "TableView")}
									...
									<div className='row mx-2' style={{textAlign: "center"}}>
										...
										<div className='col-md-2 mt-3'>
											<li className='list-group-item'>
												Service Price:
												<span className='detailsAboutAppointment'>
													EGP {singleAppointment.servicePrice}
												</span>
											</li>
										</div>
										<div className='col-md-2 mt-3'>
											<li className='list-group-item'>
												Paid Tip: {"  "}
												<span className='detailsAboutAppointment'>
													EGP {singleAppointment.paidTip.toFixed(2)}
												</span>
											</li>
										</div>
										<div className='col-md-3 mt-3'>
											<li className='list-group-item'>
												Online Service Fee:{"  "}
												<span className='detailsAboutAppointment'>
													EGP {singleAppointment.onlineServicesFees}
												</span>
											</li>
										</div>
										<div className='col-md-2 mt-3'>
