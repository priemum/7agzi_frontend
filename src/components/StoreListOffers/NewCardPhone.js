...
const NewCardPhone = ({ store, allServicesCombined }) => {
	...
	var requiredServices =
		store &&
		store.services.filter((i) => i.servicePriceDiscount < i.servicePrice);
										...
										requiredServices.map((s, i) => (
											<div key={i}>
												{chosenLanguage === "Arabic" ? (
													...
													>
														{s.serviceNameOtherLanguage.length >= 23
															? `${s.serviceNameOtherLanguage.substring(
																	0,
																	23
															  )}...`
															: s.serviceNameOtherLanguage}
														:{" "}
														<s style={{ color: "#f98787" }}>{s.servicePrice}</s>{" "}
														<span
															style={{
																color: "lightgreen",
																fontFamily: '"Droid Arabic Kufi", sans-serif',
													...
													<span
														style={{
															...
															fontSize:
																s.serviceName.length > 16 ? "10px" : "12px",
														}}
													>
														{s.serviceName.length >= 20
															? `${s.serviceName.substring(0, 20)}...`
															: s.serviceName}
														:{" "}
														<s style={{ color: "#f98787" }}>{s.servicePrice}</s>{" "}
														<span
