...
const UpdateAppointmentHelper = ({
	...
	const totalServicePriceDiscount =
		...
		) - Number(singleAppointment.discountedAmount);
	...
	return (
		<UpdateAppointmentHelperWrapper dir='ltr'>
			<>
				<div className='pt-5 pb-4'>
					...
					>
						<strong>Update Stylist/ Employee...</strong>
					</div>
				</div>
				...
				serviceDetailsArray.length > 0 ? (
					<div>
						...
						>
							Select An Appointment Time
							<br />
							<span>Employee ALL Time Slots</span>
							<br />
						</label>
						<br />
						<select
						...
						chosenTime ? (
							...
							>
								<button className='btn btn-success w-50'>UPDATE NOW</button>
							</div>
						) : null}
					</div>
				) : null}
			</>
		</UpdateAppointmentHelperWrapper>
	);
};

export default UpdateAppointmentHelper;

const UpdateAppointmentHelperWrapper = styled.div`
	text-align: center;
