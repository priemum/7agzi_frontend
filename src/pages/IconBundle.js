...
import StoreListPhone from "../components/StoreListBundle/StoreListPhone";
			...
			.then((data) => {
				if (data.error) {
					...
					var uniqueStoresWithLatestDates = data.stores.filter((store) =>
						store.services.some((service) => service.serviceName.includes("+"))
					);
		...
		<MyStoreListWrapper showPagination={selectedGovernorate}>
			<Helmet dir={chosenLanguage === "Arabic" ? "rtl" : "ltr"}>
				...
				{chosenLanguage === "Arabic" ? (
					<title dir='rtl'>اكس لوك | عروض الباقة</title>
				) : (
					<title>XLOOK | Bundle Offers</title>
				)}
				<meta
					name='description'
					content={
