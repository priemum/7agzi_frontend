...
const CheckoutGuest = ({ chosenLanguage }) => {
	...
	let deliveryCountry = data.country;
	let deliveryCity = data.city;
	let deliveryZipcode = data.zipcode;
	let deliveryState = data.state;
	...
	const buy = () => {
		...
		if (!data.city) {
			return toast.error("Please Add Your City");
		}
		if (!data.phone) {
	...
	const buyNoPayment = () => {
		...
		const createOrderData = {
			products: cart,
			transaction_id: "Pay On Delivery (Guest User)",
			amount: getTotal(),
			address: deliveryAddress,
			customerName: customerName,
			country: deliveryCountry,
			...
			minLoyaltyPointsForAward:
				...
				alreadySetLoyaltyPointsManagement.loyaltyPointsAward,
			discountedAmount: getActualTotal() - getTotal(),
			card_data: "Pay On Delivery (Guest User)",
			onlineServicesFees: alreadySetLoyaltyPointsManagement.onlineServicesFees,
			androidPayCard: "Pay On Delivery (Guest User)",
			applePayCard: "Pay On Delivery (Guest User)",
			applyPoints: false,
			email: deliveryEmailAddress,
			// zipcode: cardData.billing_postal_code,
			appliedCoupon:
					...
					<div>
						...
						<div className='row'>
							<div className='col-md-12 mx-auto'>
								<div className='form-group '>
									...
									<input
										...
										value={data.address}
										autoFocus
										placeholder='Please type your mailing Address. (e.g. : 123 main street or PO box 12345)'
										required
									/>
								</div>
							</div>
							...
							<div className='col-md-5 mx-auto'>
								<div className='form-group'>
									<label className='text-muted'>
										{chosenLanguage === "Arabic" ? "مدينة" : "City"}{" "}
										{data.city && data.city.length >= 1
											? null
											: requiredNotation()}
									</label>
									<input
										...
										className='form-control'
										value={data.city}
										placeholder='Please type your City. (e.g. : LA or Los Angeles)'
										required
									/>
								</div>
							</div>
							...
							<div className='col-md-5 mx-auto'>
								<div className='form-group'>
									<label className='text-muted'>
										{chosenLanguage === "Arabic" ? "ولاية" : "State"}{" "}
									...
									<input
										...
										className='form-control'
										value={data.state}
										placeholder='Please type your State. (e.g. : CA or California)'
										required
									/>
								</div>
							</div>
							...
							<div className='col-md-5 mx-auto'>
								<div className='form-group '>
									<label className='text-muted'>
										{chosenLanguage === "Arabic" ? "دولة" : "Country"}{" "}
									...
									<input
										...
										className='form-control'
										value={data.country}
										placeholder='Please type your Country. (e.g. : USA or United States of America) '
										required
									/>
								</div>
							</div>
							...
							<div className='col-md-5 mx-auto my-auto'>
								<div className='form-group'>
									<label className='text-muted'>
										{chosenLanguage === "Arabic" ? "رمز بريدي" : "Zip Code"}{" "}
									...
									<input
										...
										className='form-control'
										value={data.zipcode}
										placeholder='Please type your Zip Code. (e.g. : 92223)'
										required
									/>
								</div>
							</div>
									...
									<h4 style={{ fontWeight: "bold", fontSize: "1rem" }}>
										...
										<span style={{ color: "#3498ff" }}>
											{carrierDataFromStorage &&
												carrierDataFromStorage.carrierName}
										</span>
									</h4>
									<hr />
									<h4 style={{ fontWeight: "bold", fontSize: "1rem" }}>
